...
module "kube-hetzner" {
  ...
  base_domain = "cluster.proqa.gg.ax"
  ...
  # k3s_registries = <<-EOT
  #   mirrors:
  #     registry.proqa.gg.ax:
  #       endpoint:
  #         - "private-registry.default.svc.cluster.local:5000"
  # EOT
  ...
  #   },
     {
      description = "Connect to external AI service"
      direction       = "out"
      protocol        = "tcp"
      port            = "4723"
...
service:
  ...
  type: LoadBalancer
  annotations:
    "load-balancer.hetzner.cloud/name": "proqa"
...
providers:
  ...
  # The following is an example, please note that the current indentation inside the EOT is important.
  # "external-dns.alpha.kubernetes.io/hostname": "lb.cluster.proqa.gg.ax"
  nginx_values = <<EOT
controller:
  ...
  service:
    annotations:
      ...
      "load-balancer.hetzner.cloud/type": "lb11"
      "load-balancer.hetzner.cloud/uses-proxyprotocol": "true"
      "load-balancer.hetzner.cloud/hostname": "lb.cluster.proqa.gg.ax"
  EOT
